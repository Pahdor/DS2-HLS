...
processed_folder = join(get_root_path(), 'processed')
create_dir_if_not_exists(processed_folder)
src_folder = join(get_root_path(), 'sources')
graph_folder = join(get_root_path(), 'graphs')
graph_base_folder= join(graph_folder, 'base')
create_dir_if_not_exists(graph_base_folder)
...
def process_graph(name, g, csv_dict=None):
    ...
    new_gexf_file = join(graph_base_folder, f'{name}_processed_result.gexf')
...
def graph_generator(name, path, generate_programl = False, csv_dict=None):
    """
    ...
    if mode == 'initial':
        for BENCHMARK in target:
            for kernel in ALL_KERNEL[BENCHMARK]:
                logger.info('####################')
                logger.info(f'now processing {kernel}')
                ## modify the path below:
                path = join(processed_folder, f'{kernel}')
                if not exists(path):
                    create_dir_if_not_exists(path)
                    ## this file needs to be run in programl mode first for this mode to work
                    kernel_path = join(src_folder, f'{kernel}_kernel.c')
                    if not exists(kernel_path):
                        logger.error('Error: no source file found')
                        raise RuntimeError()
                    ext = kernel_path.split('.')[-1]
                    new_file_path = join(path, f'{kernel}.{ext}')
                    logger.info(f"copy source file from {kernel_path} to {new_file_path}")
                    shutil.copyfile(kernel_path, new_file_path)
                graph_generator(kernel, path, generate_programl = True, csv_dict=csv_dict)
        write_csv_file(csv_dict, csv_header, f'{mode}.csv')
    elif mode == 'auxiliary':
        for BENCHMARK in target:
            if connected:
                auxiliary_node_graph_folder = join(graph_folder, 'extended-pseudo-block-connected/')
            else:
                auxiliary_node_graph_folder = join(graph_folder, 'extended-pseudo-block-base/')
            create_dir_if_not_exists(auxiliary_node_graph_folder)
            all_kernel = ALL_KERNEL[BENCHMARK]
            for kernel in all_kernel:
                print('####################')
                print('now processing', kernel)
                add_auxiliary_nodes(kernel, graph_base_folder, auxiliary_node_graph_folder, csv_dict=csv_dict, node_type = 'block', connected = connected)
                print()
        write_csv_file(csv_dict, csv_header, f'{mode}_{connected}.csv')
    elif mode == 'hierarchy':
        for BENCHMARK in target:
            auxiliary_node_graph_folder = join(graph_folder, 'extended-pseudo-block-connected/')
            dest_path = join(graph_folder, 'extended-pseudo-block-connected-hierarchy/')
            create_dir_if_not_exists(dest_path)
            assert exists(auxiliary_node_graph_folder)
            for kernel in ALL_KERNEL[BENCHMARK]:
                print('####################')
                print('now processing', kernel)
                for_blocks_info = get_for_blocks_info(kernel, processed_folder)
                augment_graph_hierarchy(kernel, for_blocks_info, src_path = auxiliary_node_graph_folder, dst_path = dest_path, csv_dict=csv_dict)
                print()
        write_csv_file(csv_dict, csv_header, f'{mode}.csv')
    else:
        raise NotImplementedError()
