...
BOOST_TRAIN_SAVE_DIR = osp.join(SAVE_DIR, 'boost_train')
BOOST_TEST_SAVE_DIR = osp.join(SAVE_DIR, 'boost_test')
...
class MyOwnDataset(Dataset):
    def __init__(self, transform=None, pre_transform=None, data_files=None, is_train=True, train_dir=TRAIN_SAVE_DIR, test_dir=TEST_SAVE_DIR):
        if is_train:
            self.save_dir = train_dir
        else:
...
def gen_diff_dataset(is_train, Net):
    ...
    idx = 0
    save_dir = f"{SAVE_DIR}/boost_train" if is_train else f"{SAVE_DIR}/boost_test"
