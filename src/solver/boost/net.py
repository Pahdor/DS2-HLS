...
class Net(nn.Module):
    ...
    def forward(self, data, inference_mode=False):
        ...
        diff_embed1 = global_add_pool(pragma_out, batch)
        diff_embed2 = global_add_pool(kernel_out, batch)
        ...
        if FLAGS.enable_ikdl:
            diff_loss = torch.square(pragma_out - kernel_out).mean()
            loss_dict['diff_loss'] = diff_loss
            loss_dict['init_loss'] = total_loss.clone()
            total_loss += diff_loss
                    
        return out_dict, total_loss, loss_dict, gae_loss, diff_embed1, diff_embed2, out_embed
...
class SharedNet(nn.Module):
    ...
    def forward(self, data, inference_mode=False):
        ...
        if FLAGS.encode_edge and  FLAGS.gnn_type == 'transformer':
            ...
            # out = activation(self.conv_first(out, edge_index))
        else:
            out = activation(self.conv_first(x, edge_index))
        ...
        return out_embed, kernel_out, pragma_out
   
        
class MTLNet(nn.Module):
    def __init__(self, in_channels, edge_dim = 0, init_pragma_dict = None, task = FLAGS.task, num_layers = FLAGS.num_layers, D = FLAGS.D, target = FLAGS.target):
        super(Net, self).__init__()
        self.shared = SharedNet(in_channels, edge_dim, init_pragma_dict, task, num_layers, D, target)
        ...
        self.perf = MLP(in_D, self.MLP_out_dim, activation_type=FLAGS.activation,
                                        hidden_channels=hidden_channels,
                                        num_hidden_lyr=len(hidden_channels))
        
        self.lut = MLP(in_D, self.MLP_out_dim, activation_type=FLAGS.activation,
                                        hidden_channels=hidden_channels,
                                        num_hidden_lyr=len(hidden_channels))
        
        self.ff = MLP(in_D, self.MLP_out_dim, activation_type=FLAGS.activation,
                                        hidden_channels=hidden_channels,
                                        num_hidden_lyr=len(hidden_channels))

        self.bram = MLP(in_D, self.MLP_out_dim, activation_type=FLAGS.activation,
                                        hidden_channels=hidden_channels,
                                        num_hidden_lyr=len(hidden_channels))
        
        self.dsp = MLP(in_D, self.MLP_out_dim, activation_type=FLAGS.activation,
                                        hidden_channels=hidden_channels,
                                        num_hidden_lyr=len(hidden_channels))
    ...
    def forward(self, data):
        ...
        out_embed, kernel_out, pragma_out = self.shared(data)
        
        gae_loss = 0
        
        diff_embed1 = global_add_pool(pragma_out, data.batch)
        diff_embed2 = global_add_pool(kernel_out, data.batch)
        
        perf = self.perf(out_embed)
        perf_target = _get_y_with_target(data, 'perf')
        perf_loss = self.loss_function(perf, perf_target.view(-1, 1))
        
        lut = self.lut(out_embed)
        lut_target = _get_y_with_target(data, 'util-LUT')
        lut_loss = self.loss_function(lut, lut_target.view(-1, 1))
        
        ff = self.ff(out_embed)
        ff_target = _get_y_with_target(data, 'util-FF')
        ff_loss = self.loss_function(ff, ff_target.view(-1, 1))
        
        bram = self.bram(out_embed)
        bram_target = _get_y_with_target(data, 'util-BRAM')
        bram_loss = self.loss_function(bram, bram_target.view(-1, 1))
        
        dsp = self.dsp(out_embed)
        dsp_target = _get_y_with_target(data, 'util-DSP')
        dsp_loss = self.loss_function(dsp, dsp_target.view(-1, 1))
        
        total_loss = perf_loss + lut_loss + ff_loss + bram_loss + dsp_loss
        
        loss_dict = {'perf': perf_loss, 'util-LUT': lut_loss, 'util-FF': ff_loss, 'util-BRAM': bram_loss, 'util-DSP': dsp_loss}
        ...
        return  out_dict, perf_loss, lut_loss, ff_loss, bram_loss, dsp_loss, diff_loss, loss_dict, total_loss, gae_loss, diff_embed1, diff_embed2, out_embed
