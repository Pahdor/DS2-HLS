...
import data
from solver.boost.model import BoostNet
...
def inference(dataset_li, init_pragma_dict=None, model_path=FLAGS.model_path, resample=-1, model_id=0, is_train_set=False):
    ...
    model = BoostNet(num_features, edge_dim=edge_dim, init_pragma_dict=init_pragma_dict).to(FLAGS.device)
...
def train_main(train_dataset, test_dataset, pragma_dim = None):
    ...
    model = BoostNet(num_features, edge_dim=edge_dim, init_pragma_dict=pragma_dim)
    # model.load_state_dict(torch.load('/home/kzw3933/Desktop/HLS-GNN/src/logs/dse_results_v18_2025-04-25T13-39-09.137990/run1/test_model_state_dict.pth'))
...
def test(loader, tvt, model, epoch, plot_test = False, test_losses = [-1], csv_dict = None, data_list = [], is_train_set=False):
    model.eval()
    ...
    with torch.no_grad():
        for data in tqdm(loader):
            data = data.to(FLAGS.device)
            out_dict, loss, loss_dict_, gae_loss = model.to(FLAGS.device)(data, True)
