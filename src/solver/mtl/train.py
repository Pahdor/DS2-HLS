...
def gen_dataset(li):
    train_loader = DataLoader(li[0], batch_size=FLAGS.batch_size, shuffle=False, pin_memory=False, num_workers=16)
    test_loader = DataLoader(li[1], batch_size=FLAGS.batch_size, pin_memory=False, num_workers=16)  # TODO: split make sure no seen kernels in val/test
    ...
    return train_loader, test_loader, num_features, edge_dim 
...
def inference(dataset_li, init_pragma_dict=None, model_path=FLAGS.model_path, resample=-1, model_id=0, is_train_set=False):
    
    train_loader, test_loader, num_features, edge_dim = gen_dataset(dataset_li)
    ...
    if FLAGS.task == 'regression':
        ...
        test_loss, loss_dict, gae_loss, MSE_loss = test(test_set, 'test', model, 0, plot_test = True, csv_dict = csv_dict, data_list =data_list, is_train_set=is_train_set)
...
def update_total_loss(loss, data, target_list, loss_dict, loss_dict_, out_dict, total_loss, correct):
    if FLAGS.task == 'regression':
        ...
        if 'init_loss' in loss_dict_:
            if 'init_loss' not in loss_dict:
                loss_dict['init_loss'] = loss_dict_['init_loss'].item()
            else:
                loss_dict['init_loss'] += loss_dict_['init_loss'].item()
        if 'diff_loss' in loss_dict_:
            if 'diff_loss' not in loss_dict:
                loss_dict['diff_loss'] = loss_dict_['diff_loss'].item()
            else:
                loss_dict['diff_loss'] += loss_dict_['diff_loss'].item()
...
def train_main(train_dataset, test_dataset, pragma_dim = None):
    train_loader = DataLoader(train_dataset, batch_size=FLAGS.batch_size, shuffle=True, pin_memory=False, num_workers=16)
    test_loader = DataLoader(test_dataset, batch_size=FLAGS.batch_size, shuffle=False, pin_memory=False, num_workers=16)
    ...
    task_params = [
        list(model.perf.parameters()),
        list(model.lut.parameters()),
        list(model.ff.parameters()),
        list(model.dsp.parameters()),
        list(model.bram.parameters())
    ]
    # flatten task-specific parameter lists
    task_params = [p for group in task_params for p in group]

    # 2. Create the base optimizer (Adam)
    optimizer_groups = [
        {'params': shared_params, 'lr': FLAGS.lr},
        {'params': task_params, 'lr': FLAGS.lr}
    ]
    base_optimizer = torch.optim.Adam(optimizer_groups, weight_decay=FLAGS.weight_decay)

    # 3. Wrap with PCGrad for multi-task gradient conflict resolution
    optimizer = PCGrad(base_optimizer)

    # 4. Build the LR scheduler on the *base* optimizer
    num_steps = len(train_loader) * FLAGS.epoch_num
    if FLAGS.scheduler == "multistep":
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(
            base_optimizer,
            milestones=[FLAGS.epoch_num // 3],
            gamma=0.1
        )
    ...
    # 5. Build the warmup scheduler on the PCGrad-wrapped optimizer
    if FLAGS.warmup == 'linear':
        warmup_scheduler = warmup.UntunedLinearWarmup(base_optimizer)
    elif FLAGS.warmup == 'exponential':
        warmup_scheduler = warmup.UntunedExponentialWarmup(base_optimizer)
    elif FLAGS.warmup == 'radam':
        warmup_scheduler = warmup.RAdamWarmup(base_optimizer)
    else:
        warmup_scheduler = warmup.LinearWarmup(base_optimizer, 1)
    ...
    for epoch in range(FLAGS.epoch_num):
        ...
        if len(test_loader) > 0:
            ...
            saver.log_info(('Epoch: {:03d}, Train loss: {:.4f}, min train loss at epoch: {:03d}, '
                            'Test: {:.4f}, min test loss at epoch: {:03d}, Time: {}'.format(
                            epoch, loss, train_losses.index(min(train_losses)), test_loss, test_losses.index(min(test_losses)), timer.time_and_clear())))
...
def train(epoch, model, train_loader, optimizer, lr_scheduler, warmup_scheduler):
    ...
    for data in tqdm(train_loader):
        ...
        optimizer.zero_grad()
        out_dict, loss_perf, loss_lut, loss_ff, loss_bram, loss_dsp, loss_diff, loss_dict_, loss, gae_loss = model.to(FLAGS.device)(data)
        
        if loss_diff is not None:
            optimizer.pc_backward([loss_diff, loss_perf, loss_lut, loss_ff, loss_dsp, loss_bram])
        else:
            optimizer.pc_backward([loss_perf, loss_lut, loss_ff, loss_dsp, loss_bram])
...
def test(loader, tvt, model, epoch, plot_test = False, test_losses = [-1], csv_dict = None, data_list = [], is_train_set=False, is_val_set=False):
    ...
    with torch.no_grad():
        for data in tqdm(loader):
            data = data.to(FLAGS.device)
            out_dict, loss_perf, loss_lut, loss_ff, loss_bram, loss_dsp, loss_diff, loss_dict_, loss, gae_loss = model.to(FLAGS.device)(data)
            total_loss_dict = update_total_loss(loss, data, target_list, loss_dict, loss_dict_, None, total, correct)
